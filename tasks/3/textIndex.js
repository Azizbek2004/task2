...
/** Helper function to ask a question via CLI */
function askQuestion(query) {
  return new Promise(resolve => rl.question(query, resolve));
}

/** Represents a die with its face values */
class Dice {
  constructor(faces) {
    this.faces = faces;
  }
...
/** Calculates win probabilities between dice */
class ProbabilityCalculator {
  static calculateWinProbability(diceA, diceB) {
    ...
    return winCount / 36; // 6 * 6 possible outcomes
  }
}

/** Generates ASCII probability table */
class TableGenerator {
  static generateProbabilityTable(diceList) {
    const table = new Table({
      head: ['User dice v', ...diceList.map(d => d.getFaces().join(','))],
      colWidths: diceList.map(() => 15) // Adjust dynamically if needed
    });
    ...
    for (const diceA of diceList) {
      ...
      for (const diceB of diceList) {
        ...
        row.push(diceA === diceB ? `- (${prob.toFixed(4)})` : prob.toFixed(4));
      }
      table.push(row);
    }
...
class CryptoHelper {
  ...
  static generateRandomInt(max) {
    return crypto.randomInt(max); // 0 to max-1, cryptographically secure
  }

  static computeHMAC(key, message) {
    return crypto.createHmac('sha3-256', key)
      .update(Buffer.from([message])) // Single byte for small integers
      .digest('hex');
  }
}

/** Manages fair random number generation protocol */
...
class Game {
  ...
  /** Allows the user to select a die from available options */
  ...
  /** Computer selects a die randomly */
